...
def adam(
  ...
  Let :math:`\alpha` represent the learning rate and :math:`\beta_1, \beta_2`,
  :math:`\varepsilon`, :math:`\bar{\varepsilon}` represent the arguments
  ``b1``, ``b2``, ``eps`` and ``eps_root`` respectievly.
  ...
  and computes updates :math:`u_t` and new state :math:`S_{t+1}`
  ...
  .. math::
    \begin{align*}
      ...
      u_t &\leftarrow \alpha \cdot \hat{m}_t / \left({\sqrt{\hat{v}_t +
